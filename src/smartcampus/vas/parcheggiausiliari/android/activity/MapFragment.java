...
public class MapFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		 ...
		 * 
		 * @Override public void onLongPressGesture(MotionEvent event) {
		 * Log.e("", "ASDF"); IGeoPoint point =
		 * map.getProjection().fromPixels((int)event.getX(), (int)event.getY());
		 * Toast.makeText(getActivity(),OSMGeocoding .FromPointToAddress( new
		 * GeoPoint(point.getLatitudeE6(), point.getLongitudeE6()),
		 * getActivity()).get(0).getAddressLine(0) , Toast.LENGTH_LONG).show();
		 * showPopup( map, "VIA", OSMGeocoding .FromPointToAddress( new
		 * GeoPoint(point.getLatitudeE6(), point.getLongitudeE6()),
		 ...
		 * { // TODO Auto-generated method stub Log.d("DEBUG","Pressed road");
		 * return super.onLongPress(e, mapView); } }; a.addPoint(new
		 * GeoPoint(new
		 * AusiliariHelper(getActivity()).getParklist()[0].getPosition()[0],new
		 * AusiliariHelper(getActivity()).getParklist()[0].getPosition()[1]));
		...
		MyPolyline a = new MyPolyline(getActivity()) {
			...
			public boolean onSingleTapUp(MotionEvent e, MapView mapView) {
				// TODO Auto-generated method stub
				if(isOnLine(map.getProjection().fromPixels((int)e.getX(),(int)e.getX())))
					// Log.d("DEBUG", "Path Pressed");
					Toast.makeText(getActivity(), "PATH PRESSED", Toast.LENGTH_SHORT).show();
		...
		a.setPoints(list);
		FolderOverlay kmlOverlay = new FolderOverlay(getActivity());
		kmlOverlay.add(a);
